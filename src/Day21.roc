...
num_seqs = Dict.from_list [
    ...
    (('7', '8'), [">A"]),
    (('7', '9'), [">>A"]),
    (('7', 'A'), ["vv>v>A", "vv>>vA", "v>vv>A", "v>v>vA", "v>>vvA", ">vvv>A", ">vv>vA", ">v>vvA", ">>vvvA"]),
    (('8', '0'), ["vvvA"]),
    (('8', '1'), ["vv<A", "v<vA", "<vvA"]),
    (('8', '2'), ["vvA"]),
    (('8', '3'), ["vv>A", "v>vA", ">vvA"]),
    ...
    when lists is
        ...
        [hd, .. as tl] ->
            ...
            List.join_map hd |prefix|
                List.map rest_products |suffix|
                    Str.concat prefix suffix

selections : Str -> List Str
selections = |s|
    zs = Util.zip(Str.to_utf8(Str.concat("A", s)), Str.to_utf8(s))
    options = List.map zs |key| Dict.get(num_seqs, key) |> Util.unwrap
    product(options)
